...
- ??: PLAY REEL? ?TTS? ??? ???? MP4?? ??. ???? ?? ?? ??? ?? ???/??.
- ??: Rolling Buffer ???? **??? ??**? ?? (`withAudioEnabled()`).
- ?? ??: ???? ?? ?? Gemini 3 Pro? ???/????? ?? `SRT`(00:00:01,000 ??) ???? ?? ? MediaMuxer/ExoPlayer ?? ???? ???.
- ??/??: maxOutputTokens=4096 ??, ??? ?? 40? ??. ?? ?? intervalSec 20~30?? ?? ??.

### ?????
- [ ] VideoManager: `.withAudioEnabled()` ?? ?????? ?? ?? ??
- [ ] GeminiRepository: ??? `gemini-3-pro-preview`? ?? ? ???/?? ??
- [ ] ???? ?? ?? ???? PLAY REEL ??? + ?? ??
- [ ] ??: ???? ?? ? Gemini 3 Pro? `SRT` ?? ?? ? ??? ?? ?? ??
- [ ] ????/?? ??? ??(optional), ??? ???+??? ???? ??
...
핵심 컨셉: 테마 중심의 보드게임 플레이 중 발생하는 비명(Audio)과 상황 사진(Vision)을 수집하여, 게임의 세계관에 맞춘 시네마틱 하이라이트 로그를 생성합니다.
One Engine, Infinite Themes: 애프터로그의 본질은 **'범용 보드게임 중계 엔진'**입니다. Gemini에게 주입하는 **'페르소나 프롬프트'**만 변경하면, 판타지, 추리, 좀비물 등 어떤 장르로든 즉시 변환 가능한 확장성을 가집니다. (본 데모는 그 중 코스믹 호러 테마를 적용한 사례입니다.)
...
"리그 오브 레전드(LoL)는 게임이 끝나면 '내가 이때 개쩔었지(God Play)' 하고 리플레이를 돌려볼 수 있잖아요. 그 쾌감이 엄청나죠. 그런데 보드게임은? 3시간 동안 웃고 떠들어도 게임판을 접으면 모든 기억이 휘발됩니다. 기록을 남기고 싶어도 매번 받아 적을 수도 없고요. 이 간극을 AI로 메우고 싶었습니다."

이 아이디어를 실현하려면 **"게임판의 시각적 변화"**와 **"플레이어들의 육성(비명)"**을 동시에 이해하고 연결해야 합니다. 과거엔 불가능했지만, 압도적인 멀티모달 성능을 가진 Gemini 3 Pro가 등장했기 때문에 비로소 시도해볼 수 있게 되었습니다. **[Afterlog]**는 휘발되는 아날로그 경험을 영원한 디지털 '시네마틱 리플레이'로 박제해줍니다.
...
Audio	Gemini 2.5 Flash & 3.0 Pro	별도의 라이브러리 없이 오디오 파일을 통째로 Gemini에 업로드하여 화자 분리 및 맥락 분석 수행.
...
하이테크 '터미널 보고서': 단순히 영상을 보여주는 것을 넘어, 신문과 아날로그 사진으로 보여주는 감성적인 데이터 가공.
AI 페르소나 스토리텔링: 단순 요약이 아닌 '탐정이나 기자' 스타일로 게임 결과를 재해석하는 느와르 감성의 접근.
...
이 시스템은 여러 대의 카메라가 하나의 지능을 공유하는 분산형 멀티모달 시스템으로 설계되었습니다.
...
UI Layer (Jetpack Compose): Material 3 디자인을 적용한 매거진 스타일의 리플레이 뷰어.
...
화자 분리(Speaker Diarization): 목소리의 주파수 특성을 분석해 "Speaker A(웬디)", "Speaker B(레오)"로 구분합니다. 이를 통해 로그의 주체("누가 비명을 질렀나")를 명확히 합니다.
비동기 결과 생성 (Post-Processing): 방대한 녹취록과 사진을 분석하는 작업은 시간이 걸리므로, 게임 종료 후 **'분석 대기 시간(Loading View)'**을 UX적으로 풀어냅니다. (예: "탐정 사무소에서 자료를 정리 중입니다...")
이미지 리사이징: AI 분석 및 영상용 사진은 FHD(1080p)로 다운샘플링하여 처리 속도를 높입니다.
Note
...
영상: 보드게임 박스, 로고, 캐릭터 일러스트가 영상에 크게 잡히면 Blur 처리하거나, 자체 제작한 UI 위주로 촬영합니다.
AI 출력: Gemini가 특정 저작권 캐릭터 이름을 생성하지 않도록 프롬프트에서 제어합니다 (예: '하비 월터스' -> '늙은 교수').
�📦 제출물 요건 (Submission Requirements)
데모 영상: 3분 이내 필수. (3분 초과 시 앞부분만 심사됨). 심사위원 설득을 위해 영어 자막 필수.
공개 링크: APK 설치가 어렵다면 Google AI Studio 프로토타입 링크를 제출하여 심사위원이 웹에서 바로 프롬프트를 테스트해볼 수 있게 합니다.
...
🧠 왜 꼭 Gemini 3 Pro여야 하는가? (Core Justification)
"그냥 소리 큰 부분만 자르면 되는 거 아냐?" -> 아닙니다.

맥락 이해 (Context): 단순 데시벨 트리거는 "성공해서 지르는 환호"와 "망해서 지르는 비명"을 구분 못 합니다. Gemini 3는 영상과 오디오를 동시에 보고 이 비명의 '의미'를 해석해서, 진짜 공포스러운 순간만 골라냅니다.
화자 식별 (Diarization): "누가" 말했는지 알아야 로그를 씁니다. Gemini의 멀티모달 능력으로 **[영상 속 인물] + [목소리]**를 매칭합니다.
내러티브 생성 (Storytelling): 단순 편집본이 아니라, 앞뒤 상황을 **"인과관계가 있는 이야기"**로 엮어주는 건 LLM만이 할 수 있습니다. (예: "레오가 문을 열었기 때문에 -> 웬디가 비명을 질렀다"는 2분 간격의 인과를 연결).
