# 🎞️ 애프터로그 (Afterlog)
> **AI GM 시네마틱 (Survivor's Black Box)**
>
> *"멀티모달 AI가 복구한 우주 기지의 비명을 한 편의 스페이스 호러 기록으로 재구성하다"*
...
- **One Engine, Infinite Themes**: 애프터로그의 본질은 **'범용 보드게임 중계 엔진'**입니다. Gemini에게 주입하는 **'페르소나 프롬프트'**만 변경하면, 판타지, 추리, 좀비물 등 어떤 장르로든 즉시 변환 가능한 확장성을 가집니다. (본 데모는 그 중 **Space Horror** 테마를 적용한 사례입니다.)
...
- **하이테크 '터미널 보고서'**: 단순히 영상을 보여주는 것을 넘어, 해킹된 보안 터미널 형식으로 AI 나레이션과 노이즈 낀 CCTV 스틸컷을 결합한 프리미엄 UX.
- **AI 페르소나 스토리텔링**: 단순 요약이 아닌 '우주 기지 메인 AI(MOTH_ER)' 스타일로 게임 결과를 재해석하는 SF 감성의 접근.
